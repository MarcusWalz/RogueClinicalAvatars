...
# bsub -q shared_2h -J "coumSTD_d10[1-1000]" -o "/home/vaf3/avatars/Theo/out/output.%J.%I" -R "rusage[mem=200]" -u vfusaro@hms.harvard.edu R CMD BATCH --vanilla --slave ../warfarin_simulator.R output.\LSB_JOBINDEX
...
############################################
#### LSF ####
avatars = read.table("/Users/vincentfusaro/Dev/ClinicalAvatars/LPM-INR-Modeling/Datasets/Coumagen_Files/pharm_200k_dosed.txt", sep="\t", header=T)

av_per = 1	# Number of avatars per file (per run)
block = av_per - 1
...
av_out_array = processAvatar(avatars=av_sub, protocol="coumagen_pharm", initialDose=16, numDaysToSimulate=90, maxDose=15, numReplicates=numReplicates, maxTime=24, rseed=rand_sub)

#save(av_out_array, file=paste("Coumagen_std_d1_", av_index, ".RData", sep = ""))
