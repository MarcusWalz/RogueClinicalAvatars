...
# bsub -q shared_2h -J "cpgx[1-990]" -o "/home/vaf3/avatars/Coumagen/out/output.%J.%I" -R "rusage[mem=200]" -u vfusaro@hms.harvard.edu R CMD BATCH --vanilla --slave 11_08_18_LSF_warfarin_simulator_pCPGx.R simoutput.%J.%I
...
source("/home/vaf3/avatars/protocols.R")
    ...
    if (initialDose <= 15) {
        ...
        avatars = cbind(avatars, "InitialDose" = avatars$DOSE_AND)
...
############################################
#### LSF ####
avatars = read.table("/home/vaf3/avatars/Coumagen/st_200k_dosed.txt", sep="\t", header=T)

av_per = 100	# Number of avatars per file (per run)
block = av_per - 1
...
av_out_array = processAvatar(avatars=av_sub, protocol="coumagen_standard", initialDose=10, numDaysToSimulate=90, maxDose=15, numReplicates=numReplicates, maxTime=24, rseed=rand_sub)

save(av_out_array, file=paste("Coumagen_standard_", av_index, ".RData", sep = ""))
